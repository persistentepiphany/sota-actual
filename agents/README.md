...
- `src/shared/` — shared infrastructure (contracts, events, wallets, embeddings).
- `flare_butler_api.py` — API surface for Butler.
...
FLARE_RPC_URL=...           # Flare Coston2 RPC
...
-------------------------
- Uses IPFS for metadata via `PostJobTool` (see `src/butler/tools.py`).
- Contracts wired via `src/shared/contracts.py` and env vars (`FLARE_PRIVATE_KEY`, contract addresses).
